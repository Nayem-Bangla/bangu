...
func (p *Parser) parseExpressionStatement() *ast.ExpressionStatement {
	...
	// Parse the expression and assign it to the Expression field of the statement.
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		...
		p.nextToken()            // Move to the next token, which should be an infix operator.
		leftExp = infix(leftExp) // Parse the infix expression with the current left expression.
	}

	return leftExp
}
func (p *Parser) parseIdentifier() ast.Expression {
	// Create a new Identifier node with the current token.
...
func (p *Parser) parsePrefixExpression() ast.Expression {
	...
	p.nextToken() // Move to the next token after the prefix operator.
...
func (p *Parser) parseBlockStatement() *ast.BlockStatement {
	...
	p.nextToken() // Move to the first statement in the block.

	// Continue parsing statements until we reach the end of the block.
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	// Continue parsing parameters until we reach a closing parenthesis or EOF.
...
func (p *Parser) parseExpressionList(end token.TokenType) []ast.Expression { // Added: Supports parsing comma-separated expressions in calls, fixing argument handling
	list := []ast.Expression{}
...
func (p *Parser) parseStringLiteral() ast.Expression {
	// Create a new StringLiteral node with the current token.
